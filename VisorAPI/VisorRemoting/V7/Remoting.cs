...
namespace VisorRemoting.V7
{
    public class Remoting : RemotingProtocol, IRemoting
    {
        ...
        public Remoting(string id)
        {
            ...
            this.state = RemoteState.Disconnect;
        }
        ...
        void IRemoting.Connect()
        {
            try
            {
                ...
                if (sck.Connected)
                    this.state = RemoteState.Connected;
                else
                    this.state = RemoteState.Disconnect;
            }
        ...
        void IRemoting.Receive()
        {
            ...
            this.state = RemoteState.Receiving;
            ...
            try
            {
                ...
                if (BytesReceive > 0)
                {
                    this.state = RemoteState.Received;
                    ...
                    if (data.Substring(0, 4) == "(999" && data.ToString().Substring(10, 2) == "RE" && data.ToString()[32] == Convert.ToChar(13))
                    {
                        if (CheckSum(data))
                        {
                            if (data[0] == '(' && data[32] == Convert.ToChar(13))
                            {
                                ...
                                this.state = RemoteState.Sending;
                                sck.SendTimeout = 10000; //Time Out!
                                sck.Send(BytesSend);
                                this.state = RemoteState.Sent;
                            }
                        }
                    }
                }
